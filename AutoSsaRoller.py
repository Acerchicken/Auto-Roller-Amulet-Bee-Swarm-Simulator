...
def run_macro(gui_data, log_main, log_raw):
    ...
    try:
        ...
        with open(html_path, "w", encoding="utf-8") as f:
            f.write("<html><head><style>body{font-family:Arial;background:#f0f0f0}table{width:100%;border-collapse:collapse;background:white}th,td{border:1px solid #ddd;padding:8px}th{background:#4CAF50;color:white}tr:nth-child(even){background:#f2f2f2}.passives{color:#d32f2f;font-weight:bold}.stats{color:#333;font-size:0.9em}img{border:1px solid #ccc}.credits{font-size:0.8em;color:#555;margin-bottom:10px}</style></head><body><h2>SSA Roll Session Log</h2><div class='credits'>Made by spectral</div><table><tr><th style='width:50px'>Roll</th><th>Detected Stats</th><th>Screenshot</th></tr>")
    ...
    while running:
        ...
        if max_honey_raw > 0:
            ...
            if (spent_session + cost_per_roll) > max_honey_raw:
                ...
                if success_callback:
                    success_callback("Max Honey Limit Reached", "The defined Max Honey limit for this session has been reached.")
                running = False
                break
        ...
        try:
            ...
            with open(html_path, "a", encoding="utf-8") as f:
                f.write(f"<tr><td>{rolls}</td><td><div class='passives'>{p_html}</div><div class='stats'>{s_html}</div></td><td><img src='{img_filename}' width='250'></td></tr>")
...
class AmuletFrame(tk.Frame):
    def __init__(self, parent, index, remove_callback, calc_callback, master_app):
        ...
        tk.Button(top_bar, text="X", font=("Arial", 8, "bold"), fg="red", width=3, command=lambda: self.remove_callback(self)).pack(side="right")
        ...
        for p in ALL_PASSIVES:
            var = tk.BooleanVar()
            chk = tk.Checkbutton(p_frame, text=p, variable=var, command=lambda v=var: [self.check_passive_limit(v), self.calc_callback()], anchor="w")
            chk.pack(fill="x")
        ...
        for stat, (min_r, max_r) in STAT_RANGES.items():
            ...
            entry = tk.Entry(row, textvariable=entry_var, width=4, state='disabled', validate='focusout', validatecommand=vcmd)
    ...
    def set_config(self, data):
        for var in self.passive_vars.values(): var.set(False)
        saved_passives = data.get('passives', [])
        if isinstance(saved_passives, list):
            for p in saved_passives:
                if p in self.passive_vars: self.passive_vars[p].set(True)
        ...
        for s, val in stat_checks.items():
            if s in self.stat_vars:
                ...
                if s in self.stat_entries: self.stat_entries[s]['widget'].config(state=state)
        stat_values = data.get('stat_values', {})
        for s, val in stat_values.items():
            if s in self.stat_entries: self.stat_entries[s]['var'].set(val)
...
class MacroGUI:
    def __init__(self, root):
        ...
        self.load_config()
        if not self.amulets: self.add_amulet()
        self.calculate_odds()
    ...
    def create_stats_section(self, parent):
        ...
        tk.Label(content_frame, text="Honey (T):", font=("Arial", 9)).grid(row=0, column=0, sticky="w", padx=2)
        tk.Entry(content_frame, textvariable=self.honey_var, width=6).grid(row=0, column=1, sticky="w")
        self.honey_var.trace_add("write", lambda *args: self.calculate_odds())
        tk.Label(content_frame, text="Max Useable Honey (T): ", font=("Arial", 9)).grid(row=0, column=2, sticky="w", padx=2)
        tk.Entry(content_frame, textvariable=self.max_honey_var, width=6).grid(row=0, column=3, sticky="w")
        tk.Label(content_frame, text="Chance:", font=("Arial", 9)).grid(row=0, column=4, sticky="w", padx=10)
        tk.Label(content_frame, textvariable=self.var_chance, font=("Segoe UI", 9, "bold"), fg="#006600").grid(row=0, column=5, sticky="w")
        ttk.Separator(content_frame, orient='horizontal').grid(row=1, column=0, columnspan=8, sticky="ew", padx=2, pady=3)
        tk.Label(content_frame, text="Odds (Any):", font=("Arial", 9)).grid(row=2, column=0, sticky="w", padx=2)
        tk.Label(content_frame, textvariable=self.var_odds, font=("Segoe UI", 9, "bold")).grid(row=2, column=1, sticky="w")
        tk.Label(content_frame, text="Avg Time:", font=("Arial", 9)).grid(row=2, column=2, sticky="w", padx=10)
        tk.Label(content_frame, textvariable=self.var_avg, font=("Segoe UI", 9, "bold")).grid(row=2, column=3, sticky="w")
        tk.Label(content_frame, text="Runs (Ses):", font=("Arial", 9)).grid(row=2, column=4, sticky="w", padx=10)
        tk.Label(content_frame, textvariable=self.var_runs_session, font=("Segoe UI", 9, "bold")).grid(row=2, column=5, sticky="w")
        tk.Label(content_frame, text="Runs (All):", font=("Arial", 9)).grid(row=2, column=6, sticky="w", padx=10)
        tk.Label(content_frame, textvariable=self.var_runs_total, font=("Segoe UI", 9, "bold")).grid(row=2, column=7, sticky="w")
        tk.Label(content_frame, text="Est Cost:", font=("Arial", 9)).grid(row=3, column=0, sticky="w", padx=2)
        tk.Label(content_frame, textvariable=self.var_cost, font=("Segoe UI", 9, "bold")).grid(row=3, column=1, sticky="w")
        tk.Label(content_frame, text="Est Time:", font=("Arial", 9)).grid(row=3, column=2, sticky="w", padx=10)
        tk.Label(content_frame, textvariable=self.var_est_time, font=("Segoe UI", 9, "bold")).grid(row=3, column=3, sticky="w")
        tk.Label(content_frame, text="Spent (Ses):", font=("Arial", 9)).grid(row=3, column=4, sticky="w", padx=10)
        tk.Label(content_frame, textvariable=self.var_spent_session, font=("Segoe UI", 9, "bold")).grid(row=3, column=5, sticky="w")
        tk.Label(content_frame, text="Spent (All):", font=("Arial", 9)).grid(row=3, column=6, sticky="w", padx=10)
        tk.Label(content_frame, textvariable=self.var_spent_total, font=("Segoe UI", 9, "bold")).grid(row=3, column=7, sticky="w")
    def create_config_section(self, parent):
        ...
        tk.Label(grp_delay, text="Wait Stats:").pack(side="left", padx=5)
        ...
        grp_scan = tk.LabelFrame(content_frame, text="OCR Scan Area", padx=2, pady=2)
        ...
        grp_btns = tk.LabelFrame(content_frame, text="Button Click Points", padx=2, pady=2)
    ...
    def get_btn_coords(self):
        return {"yes": (self.var_yes_x.get(), self.var_yes_y.get()), "no": (self.var_no_x.get(), self.var_no_y.get())}
    def calculate_odds(self):
        ...
        for region in self.amulets:
            ...
            if len(data['passives']) >= 2: max_cost_mode = 500_000_000_000
            passives_sig = tuple(sorted(data['passives']))
            stats_sig_list = []
            for k, v in data['stats'].items(): stats_sig_list.append((k, v))
            stats_sig = tuple(sorted(stats_sig_list))
            config_signature = (passives_sig, stats_sig)
            if config_signature in seen_configs: continue 
            seen_configs.add(config_signature)
            num_passives = len(data['passives'])
            num_stats = len(data['stats'])
            p_passive = 1.0
            if num_passives == 1: p_passive = 1/6
            elif num_passives == 2: p_passive = 1/15
    ...
    def start_thread(self, targets, one_in_chance):
        ...
        if not running:
            ...
            try: max_h = float(self.max_honey_var.get())
            except: max_h = 0.0
    ...
    def load_config(self):
        ...
        try:
            ...
            if "yes" in btns: self.var_yes_x.set(btns["yes"][0]); self.var_yes_y.set(btns["yes"][1])
            if "no" in btns: self.var_no_x.set(btns["no"][0]); self.var_no_y.set(btns["no"][1])
